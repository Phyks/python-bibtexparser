...
class BibTexWriter(object):
    ...
    def __init__(self):
        #: List of BibTeX elements to write, valid values are `entries`,
        #: `comments`, `preambles`, `strings`.
        self.contents = ['comments', 'preambles', 'strings', 'entries']
        #: Character(s) for indenting BibTeX field-value pairs. Default: single
        #: space.
        self.indent = ' '
        ...
        #: Tuple of fields for ordering BibTeX entries. Set to `None` to
        #: disable sorting. Default: BibTeX key `('ID', )`.
        self.order_entries_by = ('ID', )
        #: Tuple of fields for display order in a single BibTeX entry. Fields
        #: not listed here will be displayed alphabetically at the end. Set
        #: to '[]' for alphabetical order. Default: '[]'
        self.display_order = []
    ...
    def write(self, bib_database):
        ...
        :param bib_database: bibliographic database to be converted to a BibTeX
                             string
        ...
        for content in self.contents:
            try:
                ...
                bibtex += (
                    getattr(
                        self,
                        '_{}_to_bibtex'.format(content)
                    )(bib_database)
                )
            except AttributeError:
                logger.warning(
                    ("BibTeX item '%s' does not exist and will not be "
                     "written. Valid items are %s."),
                    content,
                    self._valid_contents
                )
        return bibtex
    ...
    def _entry_to_bibtex(self, entry):
        ...
        # first those keys which are both in self.display_order and in
        # entry.keys
